...
class AlexNetDY(nn.Layer):
    def __init__(self, hash_bit=48, pretrained=True):
...
def AlexNet(hash_bit, pretrained_path):
    """
    use pretrain but no ssld
    """
    model = AlexNetDY(hash_bit)
    load_dygraph_pretrain(model, pretrained_path)
    return model
...
if __name__=="__main__":
    ...
    import numpy as np
    import paddle
    input = paddle.to_tensor(np.random.randn(64, 3, 224, 224).astype("float32"))
    out = net(input)
    print(out.shape)   #get 64 * 48
